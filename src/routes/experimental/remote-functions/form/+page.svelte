<script>
	 ...
	 */
	const guestbookForm = /** @type {any} */ (
		addGuestbookEntry.preflight(guestbookFormSchema).enhance(async ({ form, submit, data }) => {
			try {
...
<main class="space-y-8 p-4">
	<header>
		...
		<p class="preset-typo-caption">
			Svelte Remote Functions의 <code>form</code> 패턴으로 타입 안전한 폼 제출과 Optimistic UI를 구현한
			예제입니다.
		</p>
		<div class="mt-4 rounded-lg bg-surface-100-900 p-4">
			<h4 class="preset-typo-subtitle mb-2 text-primary-600-400">핵심 기능</h4>
			<ul class="space-y-2 text-sm">
				<li class="flex gap-2">
					<span class="text-primary-500">•</span>
					<span
						><code class="text-xs">preflight()</code> - 클라이언트 측 유효성 검사로 불필요한 서버 요청
						방지</span
					>
				</li>
				<li class="flex gap-2">
					<span class="text-primary-500">•</span>
					<span
						><code class="text-xs">enhance()</code> - Progressive Enhancement로 JS 없이도 동작</span
					>
				</li>
				<li class="flex gap-2">
					<span class="text-primary-500">•</span>
					<span
						><code class="text-xs">withOverride()</code> - Optimistic UI로 즉각적인 사용자 피드백</span
					>
				</li>
				<li class="flex gap-2">
					<span class="text-primary-500">•</span>
	...
	<section class="space-y-2">
		...
		<form {...guestbookForm} novalidate class=" space-y-4">
			...
			<label class="label">
				...
				{#if addGuestbookEntry.issues?.name}
					{#each addGuestbookEntry.issues.name as issue}
						<span class="text-sm text-error-500">{issue.message}</span>
					{/each}
				{:else}
			...
			<button type="submit" class="ml-auto btn block preset-filled"> 작성하기 </button>

			{#if guestbookForm.result}
				<aside
					class="alert {guestbookForm.result.success
	...
	<section class="space-y-2">
		...
		<div class="space-y-2">
			...
			{:else if entriesQuery.current?.length > 0}
				...
				{/each}
			{:else}
				<div class="py-8 text-center text-surface-400-600">
	...
	<!-- 기술 설명 섹션 -->
	<section
		class="divide-y divide-primary-300-700 card border border-primary-300-700 preset-filled-primary-200-800"
		role="note"
	>
		...
		<div class="space-y-4 p-4">
			<div>
				<h4 class="mb-2 font-semibold">사용된 기술 목록</h4>
				<ol class="list-inside list-decimal space-y-1 text-sm text-surface-600-400">
			...
			<div>
				<h4 class="mb-2 font-semibold">1. preflight(schema) - 클라이언트 사전 검증</h4>
			...
			<div>
				<h4 class="mb-2 font-semibold">2. enhance(callback) - Progressive Enhancement</h4>
				<p class="mb-2 text-sm text-surface-600-400">
					<strong>목적:</strong> JavaScript 유무와 관계없이 동작하는 견고한 폼 구현
				</p>
				<ul class="space-y-1 text-sm text-surface-600-400">
					<li>
						• <strong>JavaScript 비활성화:</strong> 전통적 HTML 폼 제출 (전체 페이지 새로고침)
					</li>
			...
			<div>
				<h4 class="mb-2 font-semibold">3. submit().updates() - Single-Flight Mutations</h4>
				<p class="mb-2 text-sm text-surface-600-400">
			...
			<div>
				<h4 class="mb-2 font-semibold">4. withOverride(callback) - Optimistic UI Updates</h4>
				<p class="mb-2 text-sm text-surface-600-400">
					<strong>목적:</strong> 네트워크 지연 시간 동안 즉각적인 UI 피드백 제공
				</p>
				<ul class="space-y-1 text-sm text-surface-600-400">
					<li>
						• <strong>성공 시:</strong> 임시 UI 업데이트 → 서버 응답 → 실제 데이터로 자동 교체
					</li>
			...
			<div>
				<h4 class="mb-2 font-semibold">실행 순서 및 데이터 흐름</h4>
				<div class="rounded bg-surface-100-900 p-3 font-mono text-sm">
			...
			<div>
				<h4 class="mb-2 font-semibold">Progressive Enhancement 철학</h4>
				<p class="text-sm text-surface-600-400">
					SvelteKit Remote Functions는 "점진적 기능 향상" 원칙을 따릅니다. 기본 HTML 폼으로 시작하여
					JavaScript가 있을 때만 고급 기능을 추가합니다. 이는 접근성, 견고성, 성능을 모두 보장하는
					현대적인 웹 개발 접근법입니다.
				</p>
			</div>
		</div>
